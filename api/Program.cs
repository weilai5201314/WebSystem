...
app.Use(async (context, next) =>
{
    ...
    // 测试专用判断条件
    if (path != "/Api")
        // if ((path == "/Api/user/SignUp" || path == "/Api/user/SignUp2" || path == "/Api/user/LogIn" || path == "/Api/user/LogIn2" || path == "/Api/user/RevertPass") && (context.Request.Method == "POST" || context.Request.Method == "PUT"))
    {
        await next.Invoke();
        return;
    }
